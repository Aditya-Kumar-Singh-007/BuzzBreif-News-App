...
export class News extends Component {
  ...
  async updateNews(page) {
    this.setState({ loading: true });
    let url = `https://cdn.jsdelivr.net/gh/Aditya-Kumar-Singh-007/BuzzBreif_Data@main/${this.props.category}.json`;
    let data = await fetch(url);
    let parsedData = await data.json();
  ...
  fetchMoreData = async () => {
    ...
    // Add a small delay to show spinner nicely
    setTimeout(() => {
      this.setState({
